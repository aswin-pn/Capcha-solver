...
class LinkedInCaptchaSolver:
        ...
        try:
            ...
            if "checkpoint/challenge" in current_url:
                ...
                if not start_clicked:
                    print("Could not find or click Start Puzzle button")
                    return False
                            ...
                            if not try_again_button:
                                try:
                                    ...
                                    for frame in arkose_frames:
                                        ...
                                        await try_again_button.click()
                                        await asyncio.sleep(5)  # Wait for new CAPTCHA to load
                                        print("🔄 New CAPTCHA loaded, solving directly...")
                                        
                                        # Now solve the new CAPTCHA directly (skip Start Puzzle button)
                                        try:
                                            # Wait longer for puzzle images to load
                                            print("Waiting for new puzzle images to load...")
                                            await asyncio.sleep(10)  # Even longer wait for new CAPTCHA
                                            ...
                                            print(f"Error solving new CAPTCHA after retry: {e}")
                                            return False
                                        except:
                                            continue
                                except Exception as e:
    ...
    async def send_captcha_to_server(self, image_bytes):
        print("Attempting to send CAPTCHA to local server...")
        try:
            files = {'image': ('captcha.png', image_bytes, 'image/png')}
            response = requests.post('http://localhost:5001/solve', files=files)
            if response.status_code == 200:
                print("CAPTCHA image sent to human server successfully")
                return True
            else:
    ...
    async def poll_for_answer(self, timeout=120):
        ...
        while time.time() - start < timeout:
            try:
                response = requests.get('http://localhost:5001/answer')
                if response.status_code == 200:
                    data = response.json()
                    answer = data.get('answer')
                    if answer is not None and str(answer).isdigit():
                        print(f"Received valid answer: {answer}")
                        return int(answer)
